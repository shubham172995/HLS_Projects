...
#define AUTOTB_TVOUT_addr  "../tv/cdatafile/c.PutValue.autotvout_addr.dat"
// wrapc file define: "data"
#define AUTOTB_TVIN_data  "../tv/cdatafile/c.PutValue.autotvin_data.dat"
// wrapc file define: "ap_return"
#define AUTOTB_TVOUT_ap_return  "../tv/cdatafile/c.PutValue.autotvout_ap_return.dat"

#define INTER_TCL  "../tv/cdatafile/ref.tcl"

// tvout file define: "addr"
#define AUTOTB_TVOUT_PC_addr  "../tv/rtldatafile/rtl.PutValue.autotvout_addr.dat"
// tvout file define: "ap_return"
#define AUTOTB_TVOUT_PC_ap_return  "../tv/rtldatafile/rtl.PutValue.autotvout_ap_return.dat"
...
unsigned int data)
{
	...
	if (wrapc_switch_file_token.good())
	{
		...
		// output port post check: "addr"
		aesl_fh.read(AUTOTB_TVOUT_PC_addr, AESL_token); // [[transaction]]
		if (AESL_token != "[[transaction]]")
		{
			exit(1);
		}
		aesl_fh.read(AUTOTB_TVOUT_PC_addr, AESL_num); // transaction number

		if (atoi(AESL_num.c_str()) == AESL_transaction_pc)
		{
			aesl_fh.read(AUTOTB_TVOUT_PC_addr, AESL_token); // data

			sc_bv<32> *addr_pc_buffer = new sc_bv<32>[1];
			int i = 0;
			...
			while (AESL_token != "[[/transaction]]")
			{
				...
				while (!no_x)
				{
					...
					if (x_found != string::npos)
					{
						if (!err)
						{
							cerr << "WARNING: [SIM 212-201] RTL produces unknown value 'X' on port 'addr', possible cause: There are uninitialized variables in the C design." << endl;
							err = true;
						}
				...
				aesl_fh.read(AUTOTB_TVOUT_PC_addr, AESL_token); // data or [[/transaction]]

				if (AESL_token == "[[[/runtime]]]" || aesl_fh.eof(AUTOTB_TVOUT_PC_addr))
				{
					exit(1);
				}
			}
					...
					// bitslice(31, 0)
					{
						...
						// celement: addr(31, 0)
						{
							...
							for (int i_0 = 0; i_0 <= 0; i_0 += 1)
							{
								...
								{
									addr_lv0_0_0_1[hls_map_index].range(31, 0) = sc_bv<32>(addr_pc_buffer[hls_map_index].range(31, 0));
									hls_map_index++;
								}
							}
						}
	...
	else
	{
		...
		// "addr"
		char* tvout_addr = new char[50];
		aesl_fh.touch(AUTOTB_TVOUT_addr);
...
// [call_c_dut] ---------->
		...
		int AESL_return = PutValue(addr, data);

		CodeState = DUMP_OUTPUTS;

		// [[transaction]]
		sprintf(tvout_addr, "[[transaction]] %d\n", AESL_transaction);
		aesl_fh.write(AUTOTB_TVOUT_addr, tvout_addr);

		sc_bv<32>* addr_tvout_wrapc_buffer = new sc_bv<32>[1];
				...
				// celement: addr(31, 0)
				{
					...
					for (int i_0 = 0; i_0 <= 0; i_0 += 1)
					{
						...
						if (&(addr[0]) != NULL) // check the null address if the c port is array or others
						{
							...
							addr_tvout_wrapc_buffer[hls_map_index].range(31, 0) = addr_tmp_mem.range(31, 0);
                                 	       hls_map_index++;
						}
					}
				}
		...
		for (int i = 0; i < 1; i++)
		{
			sprintf(tvout_addr, "%s\n", (addr_tvout_wrapc_buffer[i]).to_string(SC_HEX).c_str());
			aesl_fh.write(AUTOTB_TVOUT_addr, tvout_addr);
		}

		tcl_file.set_num(1, &tcl_file.addr_depth);
		sprintf(tvout_addr, "[[/transaction]] \n");
		aesl_fh.write(AUTOTB_TVOUT_addr, tvout_addr);

		// release memory allocation
		delete [] addr_tvout_wrapc_buffer;
