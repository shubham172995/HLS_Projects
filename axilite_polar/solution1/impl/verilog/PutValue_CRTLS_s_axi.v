...
localparam
    ...
    ADDR_ADDR_DATA_0 = 6'h18,
    ADDR_ADDR_CTRL   = 6'h1c,
    ADDR_DATA_DATA_0 = 6'h20,
    ADDR_DATA_CTRL   = 6'h24,
    WRIDLE           = 2'd0,
    WRDATA           = 2'd1,
    WRRESP           = 2'd2,
    WRRESET          = 2'd3,
...
//------------------------Local signal-------------------
    ...
    reg  [31:0]                   int_addr = 'b0;
    reg                           int_addr_ap_vld;
    reg  [31:0]                   int_data = 'b0;
    ...
    if (ACLK_EN) begin
        if (ar_hs) begin
            ...
            case (raddr)
                ...
                ADDR_ADDR_CTRL: begin
                    rdata[0] <= int_addr_ap_vld;
                end
...
always @(posedge ACLK) begin
    if (ARESET)
        int_addr_ap_vld <= 1'b0;
    else if (ACLK_EN) begin
        if (addr_ap_vld)
            int_addr_ap_vld <= 1'b1;
        else if (ar_hs && raddr == ADDR_ADDR_CTRL)
            int_addr_ap_vld <= 1'b0; // clear on read
    end
end
